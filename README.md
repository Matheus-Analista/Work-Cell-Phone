...
Separado em três arquivos — `index.html`, `styles.css` e `script.js` — sem dependências externas além do Google Fonts. Totalmente responsivo, do celular ao desktop.
...
- [ ] Trocar o número de WhatsApp: abra `script.js`, procure por `WHATSAPP_NUMBER` na primeira linha e troque `"5561XXXXXXXXX"` pelo número real (só dígitos, formato `55DDDNUMERO`). Um único lugar alimenta todos os botões do site.
